...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["chào bạn tôi là bot của Lê Minh", "Gọi tui có gì hong", "Shao zạ", "Hi, chào cậu :3", "Shao nè", "Sử dụng /callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "hello...", "What'up bro", "Sao thế công chúa", "Ahihi chào cậu", "Trả 5 chục", "Cậu có cô đơn hong , có thì kệ mie cậu đồ kh có ngiu :>>", "Cho mượn 5 chục đi", "bot dễ thương như chủ của bot ạ", "Gì zạ" ,"Chuyện gì đó", "Đừng spam em nha :<<","Dạ có em đâyy^^ yêu em hongg mà gọi<33. hmmmm", "Hí xin chàooo", "get low , get low", "Spam cút đy", "Mệt -,-", "Nói đi , sao?"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot cùi")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb:https://www.facebook.com/anhminh.levan.1 để được hỗ trợ", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };

  if ((event.body.toLowerCase() == "bot out")) {
    return api.sendMessage("Tạm biệt mọi người nhaa em đi đây ><", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };

   if ((event.body.toLowerCase() == "bot cút")) {
    return api.sendMessage("À ừ vậy thôi tao đi cảm ơn bọn mày trong thời gian qua cùng tao vui vẻ....vậy thôi tao đi đây chào chúng mày ☺😊", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };

  if ((event.body.toLowerCase() == "/bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Lê Minhh để được hỗ trợ\nLink fb nè: https://www.facebook.com/anhminh.levan.1", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  ...
  if ((event.body.toLowerCase() == "chó") || (event.body.toLowerCase() == "dog")) {
    return api.sendMessage("Chó cute của mày đây 🐶", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ơi") || (event.body.toLowerCase() == "oi")) {
    return api.sendMessage("sao nèee", threadID);
  };

  if ((event.body.toLowerCase() == "ừ") || (event.body.toLowerCase() == "u")) {
    return api.sendMessage("cục súc zạ, hong được ừ, dạ đi mới ngoan :<<", threadID);
  };

  if ((event.body.toLowerCase() == "ừa") || (event.body.toLowerCase() == "ua")) {
    return api.sendMessage("Gì mà ừa thế kia", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "dạ") || (event.body.toLowerCase() == "da")) {
    return api.sendMessage("Bạn zạ cute wa' , bạn xứng đáng có 100 người iu :3", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "đm bot") || (event.body.toLowerCase() == "dm bot")) {
    return api.sendMessage("Văn minh lên nào bạn êy \n🚬🐧", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "địt") || (event.body.toLowerCase() == "dit")) {
    return api.sendMessage("Người văn minh kh ai nói thế :)", threadID);
  };

  if ((event.body.toLowerCase() == "địt cc") || (event.body.toLowerCase() == "dit cc")) {
    return api.sendMessage("Bạn im mồm lại được rồi đấy", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "Lê Minh") || (event.body.toLowerCase() == "Minh")) {
    return api.sendMessage("Gọi daddy có gì hong zạ ", threadID);
  };

  if ((event.body.toLowerCase() == "@Lê Minhh") || (event.body.toLowerCase() == "@Minhh Lê")) {
    return api.sendMessage("Daddy đi ngủ ròi í còn tôi thôi \n🚬🐧", threadID);
  };

  if ((event.body.toLowerCase() == "bot ơi") ||  (event.body.toLowerCase() == "bot ơi")) {
    return api.sendMessage("Nói lẹ đy, emm còn phải phục vụ các box khác nữa :<<", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "buổi sáng vui vẻ nha mọi người") || (event.body.toLowerCase() == "buoi sang vui ve nha moi nguoi")) {
  ...
  if ((event.body.toLowerCase() == "chửi cc") || (event.body.toLowerCase() == "chui cc")) {
    return api.sendMessage("️Matday quá đi bạn à ><, cái này phải bị phạt :3", threadID);
  };

  if ((event.body.toLowerCase() == "hentai") || (event.body.toLowerCase() == "hentai")) {
    return api.sendMessage("Thôi ông ơi , suy giảm giống nòi người Việt đấy ", threadID);
  };

  if ((event.body.toLowerCase() == "cc cút") || (event.body.toLowerCase() == "cc cút")) {
    return api.sendMessage("Sao mày hong cút? Mà bảo tao :) mệt lồn ghê :>", threadID);
  };

  if ((event.body.toLowerCase() == "vãi") || (event.body.toLowerCase() == "vai")) {
    return api.sendMessage("Vãi gì cha ", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "link") || (event.body.toLowerCase() == "link")) {
    return api.sendMessage("Link bao nhiêu phút á cho em xem với nào 🤤", threadID);
  };

  if ((event.body.toLowerCase() == "cứu") || (event.body.toLowerCase() == "cuu")) {
    return api.sendMessage("Cứu cái gì, ngu thì chết khôn thì sống cậu chủ bảo tao thế <3", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin em là LeMinhh daddy có gì dùng lệnh /ad để biết thêm chi tiết", threadID);
  };

  if ((event.body.toLowerCase() == "chán quá") || (event.body.toLowerCase() == "chan qua")) {
    return api.sendMessage("️khong sao mà , có em đây", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "con cặc") || (event.body.toLowerCase() == "con cac")) {
    return api.sendMessage("️Trái ớt nhỏ tí mà khoe ai vậy bạn :)))", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ông LêMinh") || (event.body.toLowerCase() == "ong LeMinh")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "Bạn Minh") || (event.body.toLowerCase() == "bạn minh")) {
    return api.sendMessage("️Sao nói gì admin tôi đấy ?", threadID);
  };

  if ((event.body.toLowerCase() == "bot cute quá") || (event.body.toLowerCase() == "bot cute qua")) {
    return api.sendMessage("️dạ quá khen ạ hihi :>", threadID);
  };

  if ((event.body.toLowerCase() == "haha") || (event.body.toLowerCase() == "haha")) {
    return api.sendMessage("️Haha dui thiệt đó :>>", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "bot có buồn hong") || (event.body.toLowerCase() == "po co buon hong")) {
    return api.sendMessage("Có mọi người sao bé buồn đc <3 yêu lắm <3", threadID);
  };

  if ((event.body.toLowerCase() == "bot có yêu t hong") || (event.body.toLowerCase() == "bot co yeu t hong")) {
  ...
  if ((event.body.toLowerCase() == "bot đi ngủ đi") || (event.body.toLowerCase() == "bot di ngu di")) {
    return api.sendMessage("Tớ ngủ rùi,giờ cậu mới là người nên cần đi ngủ nè , nhớ giữ sức khỏe đó <3", threadID);
  };
